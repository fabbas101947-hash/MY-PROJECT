...
async function apiCsrf() {
    if (apiCsrfToken) return apiCsrfToken;
    const response = await fetch('api/index.php?action=csrf', { credentials: 'same-origin' });
    const data = await response.json();
    if (!response.ok || !data.csrf) throw new Error('Unable to start a secure request.');
    apiCsrfToken = data.csrf;
    return apiCsrfToken;
}
...
const sliderFigure = document.querySelector("#sliderFigure");
...
// Product card size click + add to cart
document.querySelectorAll('.product-card').forEach(card => {
  const sizes = card.querySelectorAll('.sizes span');
  
  sizes.forEach(size => {
    if(size.classList.contains('out-of-stock')) return;
    
    size.addEventListener('click', (e) => {
      e.stopPropagation();
      
      // remove active from other sizes in this card
      ...
      // Add to cart (uses your existing function)
      if (typeof addToCart === 'function') {
        addToCart({ id: id + '-' + selectedSize, name: name + ' - ' + selectedSize, price: price });
      } else {
...
// Auto hide popup after 5 sec, show again on hover
